...
export default function StaleDeals() {
  ...
  return (
    <ReportView
      ...
      lead="Открытые сделки (не выигранные и не проигранные), по которым давно не было касаний: от последней активности до даты среза прошло больше N дней — или активностей не было вовсе. Касанием считаем звонок, письмо или задачу."
      state={state}
    >
      {(data) => (
        <div className="panel">
          ...
          {data.rows.length === 0 ? (
            <p className="muted">По всем открытым сделкам были касания за последние {data.n_days} дней.</p>
          ) : (
